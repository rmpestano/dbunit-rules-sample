...
Now you just need to use *CdiCucumberTestRunner* to have `Cucumber`, `CDI` and `DBUnit` to manage database on your BDD tests.

=== Example

First we need a feature file:

.src/test/resources/search-users.feature
----
include::{featuresdir}/search-users.feature[]
----

The a dataset to prepare our database:
